...
def forecast_7_days(automl: AutoML, lifecycle_map: dict, feature_cols: list, test_csv_path: str) -> dict:
    ...
    for menu in menus:
        preds = []
        # We copy history per menu to update iterativelyå
        ...
        for step in range(1, 8):
            ...
            # 기본 가중치
            w_model = 0.60 if cv > 1.0 else 0.75
            w_dow = 0.25 if cv > 1.0 else 0.15
            # seasonal naive(1주 전) 비중 소량 추가
            lag7_val = float(feat_row["lag_7"].values[0]) if "lag_7" in feat_row.columns else (float(recent_tail["매출수량"].iloc[-7]) if len(recent_tail) >= 7 else ma7)
            w_seas = 0.10 if cv > 1.0 else 0.05
            # 허라이즌(+6,+7)에서는 모델 가중을 조금 낮추고 안정항목 가중↑
            if step >= 6:
                w_model = max(0.0, w_model - 0.10)
                w_dow += 0.05
                w_seas += 0.05
            # 남은 비중을 ma7에 할당
            w_sum_partial = w_model + w_dow + w_seas
            w_ma7 = max(0.0, 1.0 - w_sum_partial)
            # 최종 블렌딩
            yhat = (w_model * yhat_model) + (w_dow * dow_avg) + (w_seas * lag7_val) + (w_ma7 * ma7)

            # 상한선 캡(급등 방지)
            hist_max = float(recent_tail["매출수량"].max()) if len(recent_tail) > 0 else yhat
            cap_base = max(1.0, min(hist_max * 1.2, (ma7 * 3.0 if ma7 > 0 else hist_max * 1.2)))
            # 퍼센타일 기반 캡(최근 90일 95퍼센타일)
            try:
                recent_90 = recent_tail.tail(90)["매출수량"].astype(float).values
                if recent_90.size > 0:
                    p95 = float(np.percentile(recent_90, 95))
                    cap = min(cap_base, max(1.0, p95 * 1.05))
                else:
                    cap = cap_base
            except Exception:
                cap = cap_base
...
def build_submission(all_test_preds: dict) -> pd.DataFrame:
    ...
    # Optional meta-blending with previous submissions to reduce variance (horizon & venue aware)
    try:
        import re
        prev_best_path = os.path.join(OUTPUT_DIR, "0817_mljar_06003.csv")
        ...
        if os.path.exists(prev_best_path):
            best_df = pd.read_csv(prev_best_path)
        elif os.path.exists(prev_a_path):
            best_df = pd.read_csv(prev_a_path)
        other_df = pd.read_csv(prev_b_path) if os.path.exists(prev_b_path) else None
        if best_df is not None:
            cols = [c for c in sub.columns]
            num_cols = [c for c in cols if c != "영업일자"]
            high_var_venues = {"포레스트릿", "카페테리아", "화담숲카페", "화담숲주막"}
            base = sub.copy()
            base[num_cols] = base[num_cols].astype(float)
            best_aligned = best_df.reindex(columns=cols).copy()
            best_aligned[num_cols] = best_aligned[num_cols].astype(float)
            other_aligned = None
            if other_df is not None:
                other_aligned = other_df.reindex(columns=cols).copy()
                other_aligned[num_cols] = other_aligned[num_cols].astype(float)

            def extract_day(label: str) -> int:
                m = re.search(r"\+(\d)일", str(label))
                return int(m.group(1)) if m else 1

            # Row-wise blend with horizon-aware weights
            for idx, row in base.iterrows():
                day_n = extract_day(row["영업일자"])
                # default weights
                if other_aligned is None:
                    w_cur, w_best, w_other = (0.7, 0.3, 0.0)
                else:
                    w_cur, w_best, w_other = (0.6, 0.3, 0.1)
                # for day 6-7, shift weight to more stable sources
                if day_n >= 6:
                    shift = 0.10
                    w_cur = max(0.0, w_cur - shift)
                    w_best = min(1.0, w_best + shift * 0.8)
                    w_other = min(1.0, w_other + shift * 0.2) if other_aligned is not None else w_best
                denom = w_cur + w_best + (w_other if other_aligned is not None else 0.0)
                if denom <= 0:
                    continue
                w_cur /= denom
                w_best /= denom
                if other_aligned is not None:
                    w_other /= denom

                # pull corresponding rows
                label = row["영업일자"]
                try:
                    best_row = best_aligned.loc[best_aligned["영업일자"] == label, num_cols].iloc[0]
                except Exception:
                    best_row = base.loc[idx, num_cols]
                if other_aligned is not None:
                    try:
                        other_row = other_aligned.loc[other_aligned["영업일자"] == label, num_cols].iloc[0]
                    except Exception:
                        other_row = base.loc[idx, num_cols]
                # venue-specific tweak: for high variance venues, tilt slightly more to best
                hv_cols = [c for c in num_cols if any(c.startswith(v + "_") for v in high_var_venues)]
                ov_cols = [c for c in num_cols if c not in hv_cols]
                # non-high-variance columns
                blend_main = (w_cur * base.loc[idx, ov_cols] + w_best * best_row[ov_cols])
                if other_aligned is not None:
                    blend_main = blend_main + (w_other * other_row[ov_cols])
                base.loc[idx, ov_cols] = blend_main.values
                # high-variance columns: shift 0.05 from current to best when day>=6
                if hv_cols:
                    w_cur_hv, w_best_hv, w_other_hv = w_cur, w_best, (w_other if other_aligned is not None else 0.0)
                    if day_n >= 6:
                        delta = min(0.05, w_cur_hv)
                        w_cur_hv -= delta
                        w_best_hv += delta
                    denom_hv = w_cur_hv + w_best_hv + w_other_hv
                    if denom_hv > 0:
                        w_cur_hv /= denom_hv
                        w_best_hv /= denom_hv
                        if other_aligned is not None:
                            w_other_hv /= denom_hv
                    blend_hv = (w_cur_hv * base.loc[idx, hv_cols] + w_best_hv * best_row[hv_cols])
                    if other_aligned is not None:
                        blend_hv = blend_hv + (w_other_hv * other_row[hv_cols])
                    base.loc[idx, hv_cols] = blend_hv.values

            base[num_cols] = base[num_cols].round().clip(lower=0).astype(int)
            sub = base
    except Exception:
        pass
