...
def add_maintenance_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for store in df["영업장명"].unique():
        ...
        maintenance_dates = store_data[store_data["is_maintenance"] == 1]["영업일자"]
        
        for idx, row in store_data.iterrows():
            current_date = row["영업일자"]
            
            # 다음 정비일까지의 일수
            next_maintenance = maintenance_dates[maintenance_dates > current_date]
            if len(next_maintenance) > 0:
                days_before = (next_maintenance.iloc[0] - current_date).days
                df.loc[idx, "days_before_maintenance"] = days_before
            
            # 이전 정비일로부터의 일수
            prev_maintenance = maintenance_dates[maintenance_dates < current_date]
            if len(prev_maintenance) > 0:
                days_after = (current_date - prev_maintenance.iloc[-1]).days
                df.loc[idx, "days_after_maintenance"] = days_after
    
    # 정비 후 첫 영업일 여부
    df["is_first_day_after_maintenance"] = (df["days_after_maintenance"] == 1).astype(int)
...
def is_maintenance_date(date: pd.Timestamp, store: str) -> bool:
    ...
    if store == "라그로타" and date.dayofweek == 0:  # 월요일
        return True
    if store in ["화담숲주막", "화담숲카페"] and date.dayofweek == 0:  # 월요일
        return True
...
def build_future_row(menu: str, future_date: pd.Timestamp, history: pd.DataFrame, lifecycle_map: dict) -> pd.DataFrame:
    ...
    if len(store_history) > 0:
        # 정비일은 미리 정의된 규칙에 따라 판별
        def get_maintenance_dates_for_store(store_name: str, start_date: pd.Timestamp, end_date: pd.Timestamp):
            maintenance_dates = []
            
            # 전체 정비일
            for date in MAINTENANCE_DATES["전체정비"]:
                date_obj = pd.to_datetime(date)
                if start_date <= date_obj <= end_date:
                    maintenance_dates.append(date_obj)
            
            # 매장별 정기 정비일
            if store_name in ["라그로타", "화담숲주막", "화담숲카페"]:
                current_date = start_date
                while current_date <= end_date:
                    if current_date.dayofweek == 0:  # 월요일
                        maintenance_dates.append(current_date)
                    current_date += pd.Timedelta(days=1)
            
            return sorted(list(set(maintenance_dates)))

        maintenance_dates_for_store = get_maintenance_dates_for_store(store_name, future_date - timedelta(days=30), future_date + timedelta(days=30)) # 최근 30일 내 정비일 확인
        
        # 다음 정비일까지의 일수
        next_maintenance = [date for date in maintenance_dates_for_store if date > future_date]
        if len(next_maintenance) > 0:
        ...
        # 이전 정비일로부터의 일수
        prev_maintenance = [date for date in maintenance_dates_for_store if date < future_date]
        if len(prev_maintenance) > 0:
    ...
    tmp["is_first_day_after_maintenance"] = int(tmp["days_after_maintenance"] == 1)
