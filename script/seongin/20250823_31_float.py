...
# -------------------------------
# Maintenance dates (from zero sales analysis)
# -------------------------------

MAINTENANCE_DATES = {
    # 전체 정비 기간 (모든 매장 동시 정비)
    "전체정비": [
    ...
    # 매장별 정기 정비일
    "라그로타_월요일": True,      # 매주 월요일
    "화담숲주막_월요일": True,    # 매주 월요일  
    "화담숲카페_월요일": True,    # 매주 월요일
    # 장기 휴업 기간
    "화담숲주막_장기휴업": ["2023-01-01", "2024-03-28"],  # 1년 3개월
    "화담숲카페_장기휴업": ["2023-01-01", "2024-03-28"],  # 1년 3개월
}

MAINTENANCE_SET = set(pd.to_datetime(MAINTENANCE_DATES["전체정비"]).normalize())
...
def make_features(df: pd.DataFrame, lifecycle_map: dict) -> pd.DataFrame:
    ...
    df = add_maintenance_features(df)  # 새로 추가
...
def save_checkpoint(train_df, lifecycle_map, train_feat, models_by_h, blend_w, completed_horizons):
    """체크포인트 저장"""
    checkpoint = {
        'train_df': train_df,
    ...
    checkpoint_file = os.path.join(CHECKPOINT_DIR, "training_checkpoint.pkl")
    try:
        with open(checkpoint_file, 'wb') as f:
            pickle.dump(checkpoint, f)
        print(f"[INFO] Checkpoint saved at {checkpoint_file}")
    except Exception as e:
        print(f"[WARNING] Failed to save checkpoint: {e}")


def resume_training(train_df, lifecycle_map, train_feat, models_by_h, blend_w, completed_horizons):
    """체크포인트에서 훈련 재개"""
    print(f"[INFO] Resuming training from checkpoint...")
    
    # 완료되지 않은 horizon들만 훈련
    remaining_horizons = set(range(1, 8)) - completed_horizons
    
    if not remaining_horizons:
        print("[INFO] All horizons completed. Proceeding to blend weights...")
        return complete_training(train_df, lifecycle_map, train_feat, models_by_h, blend_w)
    ...
    feature_cols = [
        # 정비 관련 특성들 (중요도 높음 - 앞쪽으로 이동)
        "is_maintenance", "days_before_maintenance", "days_after_maintenance", 
        "is_first_day_after_maintenance", "is_last_day_before_maintenance", "is_monday_maintenance_store",
        # 정비일 상호작용 특성들
        "maintenance_weekend", "maintenance_holiday", "first_day_weekend", "first_day_holiday",
        "days_before_maintenance_cat", "days_after_maintenance_cat",
        # time
    ...
    print(f"[INFO] Training remaining horizons: {remaining_horizons}")
    
    for h in remaining_horizons:
        print(f"[INFO] Training horizon {h}...")
        # Target at horizon h
        ...
        # 사용 가능한 특성 컬럼만 선택
        available_feature_cols = [col for col in feature_cols if col in dfh.columns]
        Xh = dfh[available_feature_cols]
        ...
        # 정비일 데이터에 더 높은 가중치 부여
        maintenance_weight = np.where(dfh["is_maintenance"] == 1, 3.0, 1.0)  # 정비일은 3배 가중치
        first_day_after_weight = np.where(dfh["is_first_day_after_maintenance"] == 1, 2.0, 1.0)  # 정비 후 첫 영업일은 2배 가중치
        last_day_before_weight = np.where(dfh["is_last_day_before_maintenance"] == 1, 1.5, 1.0)  # 정비 전 마지막 영업일은 1.5배 가중치
        
        # 정비일 전후 특별한 가중치
        days_before = dfh["days_before_maintenance"].values
        days_after = dfh["days_after_maintenance"].values
        
        # 정비일 1-3일 전후는 더 높은 가중치
        near_maintenance_weight = np.where(
            ((days_before >= 1) & (days_before <= 3)) | 
            ((days_after >= 1) & (days_after <= 3)), 
            1.8, 1.0
        )
        
        sample_weight_h = base_w_h * venue_w_h * maintenance_weight * first_day_after_weight * last_day_before_weight * near_maintenance_weight
        ...
        completed_horizons.add(h)
        
        # 각 horizon 완료 후 체크포인트 저장
        print(f"[INFO] Completed horizon {h}, saving checkpoint...")
        save_checkpoint(train_df, lifecycle_map, train_feat, models_by_h, blend_w, completed_horizons)
    
    # 블렌딩 가중치 계산
    return complete_training(train_df, lifecycle_map, train_feat, models_by_h, blend_w)


def complete_training(train_df, lifecycle_map, train_feat, models_by_h, blend_w):
    """훈련 완료 후 블렌딩 가중치 계산"""
    print("[INFO] Computing blend weights...")
    ...
    # 최종 체크포인트 저장
    save_checkpoint(train_df, lifecycle_map, train_feat, models_by_h, blend_w, set(range(1, 8)))
...
def train_model() -> tuple:
    ...
    if os.path.exists(checkpoint_file):
        ...
        try:
            ...
            lifecycle_map = checkpoint['lifecycle_map']
            train_feat = checkpoint['train_feat']
            models_by_h = checkpoint.get('models_by_h', {})
            blend_w = checkpoint.get('blend_w', {})
            completed_horizons = checkpoint.get('completed_horizons', set())
            
            print(f"[INFO] Loaded checkpoint with {len(completed_horizons)} completed horizons")
            
            # 완료된 horizon이 있으면 계속 진행
            if completed_horizons:
                print(f"[INFO] Resuming from horizon {max(completed_horizons) + 1}")
                return resume_training(train_df, lifecycle_map, train_feat, models_by_h, blend_w, completed_horizons)
        except Exception as e:
    ...
    # 특성 컬럼에서 누락된 컬럼 제거
    available_cols = [col for col in feature_cols if col in train_feat.columns]
    missing_cols = set(feature_cols) - set(available_cols)
    if missing_cols:
        print(f"[WARNING] Missing columns: {missing_cols}")
    
    X = train_feat[available_cols]
    ...
    # 초기 체크포인트 저장
    blend_w = {}  # 빈 딕셔너리로 초기화
    save_checkpoint(train_df, lifecycle_map, train_feat, {}, blend_w, set())
    ...
    def build_blend_examples(h: int, lookback_days: int = 56):
        ...
        for menu, g in train_df.groupby("영업장명_메뉴명"):
            ...
            for d in sub["영업일자"].values:
                ...
                # 사용 가능한 특성 컬럼만 선택
                available_feature_cols = [col for col in feature_cols if col in feat.columns]
                Xb = feat[available_feature_cols]
...
def add_maintenance_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 정비일 여부
    df["is_maintenance"] = df.apply(
        lambda row: is_maintenance_date(row["영업일자"], row["영업장명"]), axis=1
    ).astype(int)
    
    # 정비일 전후 특성
    df["days_before_maintenance"] = 0
    df["days_after_maintenance"] = 0
    
    # 각 매장별로 정비일 전후 계산
    for store in df["영업장명"].unique():
        store_data = df[df["영업장명"] == store].copy()
        store_data = store_data.sort_values("영업일자")
        
        # 실제 정비일 찾기 (매출수량이 0인 날짜들)
        zero_sales_dates = store_data[store_data["매출수량"] == 0]["영업일자"]
        maintenance_dates = []
        
        for date in zero_sales_dates:
            if is_maintenance_date(date, store):
        ...
        for idx, row in store_data.iterrows():
            ...
            # 다음 정비일까지의 일수
            next_maintenance = [date for date in maintenance_dates if date > current_date]
            if len(next_maintenance) > 0:
                days_before = (next_maintenance[0] - current_date).days
                df.loc[idx, "days_before_maintenance"] = days_before
            
            # 이전 정비일로부터의 일수
            prev_maintenance = [date for date in maintenance_dates if date < current_date]
            if len(prev_maintenance) > 0:
                days_after = (current_date - prev_maintenance[-1]).days
                df.loc[idx, "days_after_maintenance"] = days_after
    
    # 정비 후 첫 영업일 여부 (정비일 다음날이면서 매출수량 > 0)
    df["is_first_day_after_maintenance"] = 0
    for store in df["영업장명"].unique():
        store_data = df[df["영업장명"] == store].copy()
        store_data = store_data.sort_values("영업일자")
        
        for i in range(1, len(store_data)):
            prev_row = store_data.iloc[i-1]
            curr_row = store_data.iloc[i]
            
            # 이전 날이 정비일이고 현재 날이 정비일이 아니면서 매출이 있는 경우
            if (is_maintenance_date(prev_row["영업일자"], store) and 
                not is_maintenance_date(curr_row["영업일자"], store) and
                curr_row["매출수량"] > 0):
                
                df.loc[curr_row.name, "is_first_day_after_maintenance"] = 1
    
    # 정비 전 마지막 영업일 여부
    df["is_last_day_before_maintenance"] = (df["days_before_maintenance"] == 1).astype(int)
    
    # 정비 주기 특성 (월요일 정기 정비 매장)
    df["is_monday_maintenance_store"] = df["영업장명"].isin(["라그로타", "화담숲주막", "화담숲카페"]).astype(int)
    
    # 정비일 특성 상호작용 추가
    df["maintenance_weekend"] = (df["is_maintenance"] & df["is_weekend"]).astype(int)
    df["maintenance_holiday"] = (df["is_maintenance"] & df["is_holiday"]).astype(int)
    df["first_day_weekend"] = (df["is_first_day_after_maintenance"] & df["is_weekend"]).astype(int)
    df["first_day_holiday"] = (df["is_first_day_after_maintenance"] & df["is_holiday"]).astype(int)
    
    # 정비일 전후 기간별 특성
    df["days_before_maintenance_cat"] = pd.cut(df["days_before_maintenance"], 
                                              bins=[0, 1, 3, 7, 14, 999], 
                                              labels=[0, 1, 2, 3, 4], 
                                              include_lowest=True).astype(int)
    df["days_after_maintenance_cat"] = pd.cut(df["days_after_maintenance"], 
                                             bins=[0, 1, 3, 7, 14, 999], 
                                             labels=[0, 1, 2, 3, 4], 
                                             include_lowest=True).astype(int)
...
def is_maintenance_date(date: pd.Timestamp, store: str) -> bool:
    """정비일 여부를 확인하는 함수"""
    date_normalized = date.normalize()
    
    # 전체 정비일 체크
    if date_normalized in MAINTENANCE_SET:
        return True
    
    # 장기 휴업 기간 체크 (월요일 정기 정비보다 우선)
    if store in ["화담숲주막", "화담숲카페"]:
        start_date = pd.to_datetime(MAINTENANCE_DATES["화담숲주막_장기휴업"][0])
        end_date = pd.to_datetime(MAINTENANCE_DATES["화담숲주막_장기휴업"][1])
        if start_date <= date_normalized <= end_date:
            return True
    
    # 매장별 정기 정비일 체크 (장기 휴업이 아닌 경우만)
    if store == "라그로타" and date.dayofweek == 0:  # 월요일
        return True
    if store in ["화담숲주막", "화담숲카페"] and date.dayofweek == 0:  # 월요일 (장기 휴업 이후)
        # 장기 휴업 이후의 월요일만 정비일로 처리
        end_date = pd.to_datetime(MAINTENANCE_DATES["화담숲주막_장기휴업"][1])
        if date_normalized > end_date:
            return True
...
def build_future_row(menu: str, future_date: pd.Timestamp, history: pd.DataFrame, lifecycle_map: dict) -> pd.DataFrame:
    ...
    tmp["is_maintenance"] = int(is_maintenance_date(future_date, store_name))
    
    # 정비일 전후 계산 - 훈련 시와 동일한 방식으로 계산
    # 과거 데이터에서 실제 정비일을 찾아서 계산
    store_history = history[history["영업장명_메뉴명"].str.startswith(store_name + "_")]
    
    if len(store_history) > 0:
        # 과거 데이터에서 정비일 찾기 (매출수량이 0인 날짜들)
        zero_sales_dates = store_history[store_history["매출수량"] == 0]["영업일자"]
        maintenance_dates = []
        
        for date in zero_sales_dates:
            if is_maintenance_date(pd.to_datetime(date), store_name):
                maintenance_dates.append(pd.to_datetime(date))
        
        maintenance_dates = sorted(maintenance_dates)
        
        # 다음 정비일까지의 일수
        next_maintenance = [date for date in maintenance_dates if date > future_date]
        if len(next_maintenance) > 0:
            tmp["days_before_maintenance"] = int((next_maintenance[0] - future_date).days)
        else:
            # 미래 정비일이 없으면 최대값 설정
            tmp["days_before_maintenance"] = 999
        
        # 이전 정비일로부터의 일수
        prev_maintenance = [date for date in maintenance_dates if date < future_date]
        if len(prev_maintenance) > 0:
            tmp["days_after_maintenance"] = int((future_date - prev_maintenance[-1]).days)
        else:
            # 과거 정비일이 없으면 최대값 설정
            tmp["days_after_maintenance"] = 999
    else:
        tmp["days_before_maintenance"] = 999
        tmp["days_after_maintenance"] = 999
    
    # 정비 후 첫 영업일 여부 (정비일 다음날이면서 매출수량 > 0)
    tmp["is_first_day_after_maintenance"] = 0
    if len(store_history) > 0:
        # 정비일 전날이 정비일이고 현재 날이 정비일이 아닌 경우
        prev_date = future_date - timedelta(days=1)
        if (is_maintenance_date(prev_date, store_name) and 
            not is_maintenance_date(future_date, store_name)):
            tmp["is_first_day_after_maintenance"] = 1
    
    tmp["is_last_day_before_maintenance"] = int(tmp["days_before_maintenance"] == 1)
    tmp["is_monday_maintenance_store"] = int(store_name in ["라그로타", "화담숲주막", "화담숲카페"])

    # 정비일 특성 상호작용 추가 (훈련 시와 동일)
    tmp["maintenance_weekend"] = int(tmp["is_maintenance"].values[0] and tmp["is_weekend"].values[0])
    tmp["maintenance_holiday"] = int(tmp["is_maintenance"].values[0] and tmp["is_holiday"].values[0])
    tmp["first_day_weekend"] = int(tmp["is_first_day_after_maintenance"].values[0] and tmp["is_weekend"].values[0])
    tmp["first_day_holiday"] = int(tmp["is_first_day_after_maintenance"].values[0] and tmp["is_holiday"].values[0])
    
    # 정비일 전후 기간별 특성
    days_before = tmp["days_before_maintenance"].values[0]
    days_after = tmp["days_after_maintenance"].values[0]
    
    if days_before <= 1:
        tmp["days_before_maintenance_cat"] = 0
    elif days_before <= 3:
        tmp["days_before_maintenance_cat"] = 1
    elif days_before <= 7:
        tmp["days_before_maintenance_cat"] = 2
    elif days_before <= 14:
        tmp["days_before_maintenance_cat"] = 3
    else:
        tmp["days_before_maintenance_cat"] = 4
    
    if days_after <= 1:
        tmp["days_after_maintenance_cat"] = 0
    elif days_after <= 3:
        tmp["days_after_maintenance_cat"] = 1
    elif days_after <= 7:
        tmp["days_after_maintenance_cat"] = 2
    elif days_after <= 14:
        tmp["days_after_maintenance_cat"] = 3
    else:
        tmp["days_after_maintenance_cat"] = 4
...
def forecast_7_days(automl_or_models, lifecycle_map: dict, feature_cols: list, test_csv_path: str, blend_w: dict | None = None) -> dict:
    ...
    for menu in menus:
        ...
        for step in range(1, 8):
            ...
            # 정비일 후처리 제거 - 모델이 정비일 특성을 학습하도록 함
            # if is_maintenance_date(future_date, store_name):
            #     preds.append(0)
            #     continue
            
            feat_row = build_future_row(menu, future_date, menu_hist_full, lifecycle_map)
            # 사용 가능한 특성 컬럼만 선택
            available_feature_cols = [col for col in feature_cols if col in feat_row.columns]
            Xf = feat_row[available_feature_cols].copy()
            ...
            # 정비일 후처리 보정 제거 - 모델이 학습하도록 함
            # if is_maintenance_date(future_date - timedelta(days=1), store_name):
            #     yhat_model *= 1.1
...
def build_submission(all_test_preds: dict) -> pd.DataFrame:
            ...
            if row_label in sub["영업일자"].values:
                ...
                for menu, preds in menu_to_preds.items():
                    if menu in sub.columns:
                        sub.at[ridx, menu] = float(preds[day - 1])  # 실수로 저장
    ...
    # Ensure floats and non-negative
    for c in sub.columns:
        if c == "영업일자":
            continue
        sub[c] = sub[c].fillna(0).astype(float).clip(lower=0)  # 실수로 유지
    return sub
...
def main():
    ...
    # 체크포인트 파일 경로
    prediction_checkpoint_file = os.path.join(CHECKPOINT_DIR, "prediction_checkpoint.pkl")
    
    # 예측 체크포인트가 있는지 확인
    if os.path.exists(prediction_checkpoint_file):
        print("[INFO] Found prediction checkpoint. Loading...")
        try:
            with open(prediction_checkpoint_file, 'rb') as f:
                pred_checkpoint = pickle.load(f)
            
            models_by_h = pred_checkpoint['models_by_h']
            lifecycle_map = pred_checkpoint['lifecycle_map']
            feature_cols = pred_checkpoint['feature_cols']
            blend_w = pred_checkpoint['blend_w']
            all_test_preds = pred_checkpoint.get('all_test_preds', {})
            completed_test_files = pred_checkpoint.get('completed_test_files', set())
            
            print(f"[INFO] Loaded prediction checkpoint with {len(completed_test_files)} completed test files")
            
            # 완료되지 않은 테스트 파일들만 예측
            test_files = sorted(glob.glob(os.path.join(TEST_DIR, "TEST_*.csv")))
            remaining_test_files = [tf for tf in test_files if os.path.splitext(os.path.basename(tf))[0] not in completed_test_files]
            
            if remaining_test_files:
                print(f"[INFO] Resuming prediction for {len(remaining_test_files)} remaining test files...")
                for tf in remaining_test_files:
                    ...
                    completed_test_files.add(test_id)
                    
                    # 각 테스트 파일 완료 후 체크포인트 저장
                    save_prediction_checkpoint(models_by_h, lifecycle_map, feature_cols, blend_w, all_test_preds, completed_test_files)
            else:
                print("[INFO] All test files already completed.")
        except Exception as e:
            print(f"[WARNING] Failed to load prediction checkpoint: {e}")
            print("[INFO] Starting fresh training and prediction...")
            models_by_h, lifecycle_map, feature_cols, blend_w = train_model()
            all_test_preds = {}
            completed_test_files = set()
    else:
...
def save_prediction_checkpoint(models_by_h, lifecycle_map, feature_cols, blend_w, all_test_preds, completed_test_files):
    """예측 체크포인트 저장"""
    checkpoint = {
        'models_by_h': models_by_h,
        'lifecycle_map': lifecycle_map,
        ...
        'completed_test_files': completed_test_files,
        'timestamp': datetime.now().isoformat()
    }
    
    checkpoint_file = os.path.join(CHECKPOINT_DIR, "prediction_checkpoint.pkl")
    try:
        with open(checkpoint_file, 'wb') as f:
            pickle.dump(checkpoint, f)
        print(f"[INFO] Prediction checkpoint saved")
    except Exception as e:
        print(f"[WARNING] Failed to save prediction checkpoint: {e}")
...
def cleanup_checkpoints():
    ...
    try:
        checkpoint_files = [
            os.path.join(CHECKPOINT_DIR, "training_checkpoint.pkl"),
            os.path.join(CHECKPOINT_DIR, "prediction_checkpoint.pkl")
        ]
        
        for file_path in checkpoint_files:
            if os.path.exists(file_path):
                os.remove(file_path)
                print(f"[INFO] Cleaned up checkpoint: {file_path}")
    except Exception as e:
