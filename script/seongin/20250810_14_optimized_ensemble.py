...
# ==============================================================================
# 🎯 최종 권장 전략: 최적화된 가중 앙상블 모델 (Phase 3: 앙상블 최적화)
# - LGBM과 Transformer 모델에 가중치를 부여하여 앙상블 (예: LGBM 60%, TF 40%)
# - 평가 가중치가 높은 '담화', '미라시아'에 별도의 특화 가중치 적용
# - 대회 규칙(28일 고정 윈도우, Data Leakage 방지)을 엄격히 준수
# ==============================================================================

# 가중 앙상블 설정
WEIGHT_CONFIG = {
    # 기본 가중치: [LightGBM 가중치, Transformer 가중치]
    "default": [0.6, 0.4],
    # 업장별 특화 가중치
    "special": {
        "담화": [0.5, 0.5],    # 담화는 두 모델을 동등하게
        "미라시아": [0.4, 0.6]  # 미라시아는 Transformer에 더 높은 가중치
    }
}
...
class RestaurantDemandPredictor:
    ...
    def predict_7days_ensemble(self, data_28days, last_date, menu_name):
        """LGBM과 Transformer 예측 결과를 가중 앙상블하여 7일 예측"""
        # 1. LightGBM 예측
        ...
        ensemble_preds = (np.array(lgbm_preds) * weights[0] + transformer_preds * weights[1])
...
if __name__ == "__main__":
    ...
    if all_predictions:
        ...
        output_filename = 'submission_optimized_ensemble.csv'
    ...
    print("\n=== 🏆 최적화된 가중 앙상블 모델 ===")
    print(f"✅ 기본 가중치 (LGBM, TF): {WEIGHT_CONFIG['default']}")
    print(f"✅ 특화 가중치: {WEIGHT_CONFIG['special']}")
