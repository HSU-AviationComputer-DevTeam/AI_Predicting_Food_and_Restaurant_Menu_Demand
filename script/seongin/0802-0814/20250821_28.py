...
try:
    ...
    print("[INFO] mljar-supervised not found in current venv.")
    raise
...
OUTPUT_DIR = BASE_ROOT / "mljar_results"  # 결과 저장 폴더
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

RULE_CSV = TRAIN_DIR / "assoc_rules_by_venue.csv"  # 연관규칙 파일 경로(사용 시)
...
def train_model() -> tuple:
    ...
    results_dir = OUTPUT_DIR / f"mljar_{run_tag}"
    ...
    for h in range(1, 8):
        ...
        kwargs["results_path"] = str(results_dir / f"h{h}")
    ...
    with open(str(OUTPUT_DIR / "mljar_inference_config.json"), "w", encoding="utf-8") as f:
...
def adjust_with_rules(pred_df, rules_map, alpha=0.3, min_trigger=1):
    """
    예측 결과에 연관규칙을 적용하여 보정
    pred_df: 영업일자, 영업장명_메뉴명, 매출수량 컬럼을 가진 DataFrame
    """
    import numpy as np, pandas as pd
    
    # pred_df가 올바른 형태인지 확인
    ...
    for (day, venue), g in df.groupby(["영업일자","venue"]):
        # 정비 기간이면 보정하지 않음
        day_date = pd.to_datetime(day.split('+')[0])  # TEST_00+1일 -> TEST_00 날짜 추출
...
def main():
    ...
    sub = build_submission(adjusted_test_preds)  # 보정된 예측 사용
