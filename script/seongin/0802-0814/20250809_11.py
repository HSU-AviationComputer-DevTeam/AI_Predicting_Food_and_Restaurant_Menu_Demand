...
if not test_paths:
    ...
    for menu_name in tqdm(unique_menus, desc="메뉴별 모델 학습 및 예측"):
        ...
        predictor_path = f'autogluon_models_gpu_3060ti/{menu_name.replace("/", "_").replace(" ", "")}'
        ...
        if os.path.exists(predictor_path):
            ...
            predictor = TabularPredictor(
                ...
                train_data_ag,
                time_limit=600,  # 메뉴별 최대 학습 시간을 600초로 늘림
                hyperparameters={
                    'CAT': {},
                    'GBM': {},  # LightGBM (잘못된 'LGB' 키를 'GBM'으로 수정)
                    'XGB': {},
                    'NN_TORCH': {}, # 딥러닝 모델 추가 (GPU 활용도 높음)
                },
                # 로컬 3060 Ti GPU 1개 사용 설정
