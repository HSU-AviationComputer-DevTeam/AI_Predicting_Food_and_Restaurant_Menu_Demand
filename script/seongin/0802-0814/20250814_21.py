...
if not test_paths:
    ...
    for menu_name in tqdm(unique_menus, desc="메뉴별 모델 학습 및 예측"):
        ...
        predictor = None
        # "스마트한 이어하기" 기능: 불완전한 모델은 삭제 후 재학습
        ...
        if predictor is None:
            ...
            hyperparameters = {
                'GBM': [
                    ...
                    {'num_leaves': 63, 'learning_rate': 0.03, 'feature_fraction': 0.85,
                     'bagging_fraction': 0.75, 'bagging_freq': 7, 'verbose': -1}
                ],
                'CAT': [
                    {'loss_function': 'RMSE', 'learning_rate': 0.05, 'depth': 6,
                     'l2_leaf_reg': 3, 'border_count': 128, 'thread_count': -1},
                    {'loss_function': 'RMSE', 'learning_rate': 0.08, 'depth': 8,
                     'l2_leaf_reg': 1, 'border_count': 254, 'thread_count': -1}
                ],
                'XGB': [
                    {'learning_rate': 0.05, 'max_depth': 6, 'subsample': 0.8,
                     'colsample_bytree': 0.9, 'reg_alpha': 0.1, 'reg_lambda': 1},
                    {'learning_rate': 0.03, 'max_depth': 8, 'subsample': 0.75,
                     'colsample_bytree': 0.85, 'reg_alpha': 0.5, 'reg_lambda': 2}
                ],
                'RF': [
            ...
            predictor = TabularPredictor(
                ...
                time_limit=300, # 메뉴별 최대 학습 시간(초) - 성능 향상
                presets='medium_quality',  # 속도와 성능 균형
                num_stack_levels=2,  # 앙상블 레벨 설정
                num_bag_folds=5,     # 배깅 폴드 수
        ...
        for path in test_paths:
            ...
            for i in range(7):
                ...
                pred_ag = predictor.predict(X_test_aligned).iloc[0]
                pred_ag = max(0, pred_ag)

                # Prophet 보조 예측값 (날짜 정렬 기준)
                pred_prophet = None
                if menu_name in prophet_model_cache and Prophet is not None:
                    pred_prophet = get_prophet_yhat_for_date(menu_name, pd.to_datetime(next_date))

                # 앙상블
                if (pred_prophet is not None) and np.isfinite(pred_prophet):
                    pred_final = (1 - PROPHET_WEIGHT) * pred_ag + PROPHET_WEIGHT * pred_prophet
                else:
                    pred_final = pred_ag
